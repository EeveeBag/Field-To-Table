...
import type { useMenuSet } from '@/features/menuSet/hooks/useMenuSet'

type MenuSetItem = NonNullable<ReturnType<typeof useMenuSet>['data']>[number]
      ...
      <div className="flex justify-between items-center">
        <div className="flex-1">
          ...
          <p className="text-earthTone-200 text-sm">
            主菜 {menuSet.dishes.filter((d) => d.type === 'main').length}，副菜{' '}
            {menuSet.dishes.filter((d) => d.type === 'side').length}
          </p>
        </div>
        <div className="flex items-center gap-3 ml-4">
          <div className="px-4 py-2 bg-earthTone-100 rounded-full">
            <span className="text-content text-sm whitespace-nowrap">{menuSet.servings} 人份</span>
          </div>
